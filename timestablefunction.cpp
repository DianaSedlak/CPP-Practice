...
// Print the column labels for n x n multiplication table.
void col_numbers(int n) {
    std::cout << "      ";
    for (int column = 1; column <= n; column++)
        std::cout << std::setw(4) << colum; // Print heading for this column.
    std::cout << '\n';
}
...
void col_line(int n) {
    std::cout <<"      +";
    for (int column = 1; column <= n; column++)
        std::cout << "----";         // Print seperator for this row.
    std::cout << '\n';
}

// Print the title of each column across the top of the table
// Including the line seperator
...
// Print a multiplication table the size of n x n.
...
int main() {
    // Get table size from user; allow values in the range 1 through 18
    int size = get_int_range(1, 18);

    // Print a size x multiplication table
    timestable(size);
}
